...

{
  ...
  for (  i  in 1:length(files)){
    ...
    bm_plotDS(arr[store[24]:dim(arr)[1],3,i], total.bins = 200, plot.error = F)
  }
  bm_plotDS(rowMeans(arr[,3,], na.rm = T), total.bins = 200, plot.error = F)
...
per.min(flaps = arr[,3,1], time = as.integer(arr[,4,1]))
